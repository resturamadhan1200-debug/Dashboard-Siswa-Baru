...
if df is not None and not df.empty:
    # Pre-processing Kolom Soal (Isi data kosong dengan 0 agar K-Means tidak error)
    soal_cols = [col for col in df.columns if "soal" in col.lower()]
    df[soal_cols] = df[soal_cols].fillna(0) 
    
    df['Skor_Total'] = df[soal_cols].sum(axis=1)
    
    # Buat ID Siswa jika belum ada kolom Nama/Siswa
    if 'Siswa_ID' not in df.columns:
    ...
    if mode_view == "Global Insight":
        ...
        with t2:
            ...
            # Mencegah error jika sampel kurang dari n_clusters
            n_clusters = min(3, len(df)) 
            kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init=10)
            
            # Ubah tipe data cluster menjadi string agar Plotly menganggapnya diskrit (kategori)
            df['Cluster'] = kmeans.fit_predict(df[soal_cols]).astype(str)
        ...
        with col_s1:
            # Mengatur nilai maksimum gauge dinamis berdasarkan jumlah soal (asumsi 1 soal max 5 poin, ubah jika beda)
            max_skor = len(soal_cols) * 5 
            ...
            fig_gauge = go.Figure(go.Indicator(
                ...
                gauge = {'axis': {'range': [0, max_skor]},
                         'bar': {'color': "#00DBDE"},
                         'steps': [
                             {'range': [0, max_skor * 0.4], 'color': "rgba(255,0,0,0.1)"},
                             {'range': [max_skor * 0.4, max_skor * 0.7], 'color': "rgba(255,255,0,0.1)"},
                             {'range': [max_skor * 0.7, max_skor], 'color': "rgba(0,255,0,0.1)"}]}
            ))
            fig_gauge.update_layout(paper_bgcolor='rgba(0,0,0,0)', font={'color': "white"})
            st.plotly_chart(fig_gauge, use_container_width=True)

        with col_s2:
            # Line Chart / Performa per soal
            ...
            fig_line.update_layout(xaxis_title="Nomor Soal", yaxis_title="Poin", paper_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig_line, use_container_width=True)

else:
    st.error("❌ File 'data.xlsx' tidak terdeteksi, atau formatnya tidak sesuai/kosong!")
    st.info("💡 Pastikan file Excel berada di folder yang sama dengan script ini dan memiliki kolom berisi kata 'soal' (misal: soal_1, soal_2).")
